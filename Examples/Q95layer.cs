 ...
 * that he has got two pegs in correct color and position (but will 
 * not tell him that they are "R" and "Y") and that he has got one peg 
 * in the correct color but incorrect position (but will not tell 
 * him that it's "B"). 
 * The player has at most 12 turns. If he can find out the
...
class MasterMind {
    ...
    MasterMind() {
        ...
        for (i = 0; i < 12;) { //can guess at most 12 times
            ...
            int n = 0; //how many pegs are right in color but 
                       //wrong position.
            ...
            for (j = 0; j < currentGuess.length();) {
                ...
                k = currentSecret.indexOf(currentGuess.charAt(j));
                if (k!=-1) {
                    n++;
                    //delete the peg.
                    currentGuess=currentGuess.substring(0, j)+
            ...
            Console.WriteLine(m+" are right in color and position");
            Console.WriteLine(n+" are right in color but wrong position");
            if (m==4) { //all found?
